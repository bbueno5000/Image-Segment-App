package com.example.imagesegmentation.task_api
...
class ImageSegmentationModelExecutor(context: Context, private var useGPU: Boolean = false) {
    ...
    init {
        ...
        imageSegmenter = ImageSegmenter.createFromFileAndOptions(
            context, IMAGE_SEGMENTATION_MODEL, options)
    }
    ...
    private fun createMaskBitmapAndLabels(
        result: Segmentation, inputWidth: Int, inputHeight: Int): Pair<Bitmap, Map<String, Int>> {
        ...
        for (coloredLabel in coloredLabels) {
            ...
            colors[cnt++] = Color.argb(
                ALPHA_VALUE, Color.red(rgb), Color.green(rgb), Color.blue(rgb))
        }
        ...
        val maskBitmap = Bitmap.createBitmap(
            pixels, maskTensor.getWidth(), maskTensor.getHeight(), Bitmap.Config.ARGB_8888)
        // Scale the maskBitmap to the same size as the input image.
        return Pair(Bitmap.createScaledBitmap(
            maskBitmap, inputWidth, inputHeight, true), itemsFound)
    }
    ...
    fun execute(inputImage: Bitmap): ModelExecutionResult {
        try {
            ...
            val (maskBitmap, itemsFound) = createMaskBitmapAndLabels(
                results.get(0), inputImage.getWidth(), inputImage.getHeight())
            ...
            return ModelExecutionResult(stackTwoBitmaps(
                maskBitmap, inputImage), inputImage, maskBitmap,
            ...
            val emptyBitmap = Bitmap.createBitmap(
                inputImage.getWidth(), inputImage.getHeight(), Bitmap.Config.ARGB_8888)
    ...
    private fun stackTwoBitmaps(foregrand: Bitmap, background: Bitmap): Bitmap {
        val mergedBitmap = Bitmap.createBitmap(
            foregrand.getWidth(), foregrand.getHeight(), foregrand.getConfig())
