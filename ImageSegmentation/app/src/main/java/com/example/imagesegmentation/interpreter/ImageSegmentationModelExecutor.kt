package com.example.imagesegmentation.interpreter
...
import com.example.imagesegmentation.utils.ImageUtils
 ...
 * More information about the DeepLab model can be found here:
...
class ImageSegmentationModelExecutor(context: Context, private var useGPU: Boolean = false) {
    ...
    init {
        ...
        segmentationMasks = ByteBuffer.allocateDirect(
            1 * imageSize * imageSize * NUM_CLASSES * 4)
        ...
        init {
            ...
            for (i in 1 until NUM_CLASSES) {
                segmentColors[i] = Color.argb(
                    (128), getRandomRGBInt(random),
                    getRandomRGBInt(random), getRandomRGBInt(random))
            }
        }
    ...
    private fun convertBytebufferMaskToBitmap(
        inputBuffer: ByteBuffer, imageWidth: Int, imageHeight: Int, backgroundImage: Bitmap,
        colors: IntArray): Triple<Bitmap, Bitmap, Map<String, Int>> {
        ...
        val scaledBackgroundImage = ImageUtils.scaleBitmapAndKeepRatio(
            ...
            for (x in 0 until imageWidth) {
                ...
                for (c in 0 until NUM_CLASSES) {
                    val value = inputBuffer.getFloat((
                            y * imageWidth * NUM_CLASSES + x * NUM_CLASSES + c) * 4)
                    if (c == 0 || value > maxVal) {
                        maxVal = value
                        mSegmentBits[x][y] = c
                    }
                ...
                val newPixelColor = ColorUtils.compositeColors(
                    colors[mSegmentBits[x][y]], scaledBackgroundImage.getPixel(x, y))
    ...
    fun execute(data: Bitmap): ModelExecutionResult {
        try {
            ...
            val contentArray = ImageUtils.bitmapToByteBuffer(
                scaledBitmap, imageSize, imageSize, IMAGE_MEAN, IMAGE_STD)
            ...
            val (maskImageApplied, maskOnly, itemsFound) = convertBytebufferMaskToBitmap(
                segmentationMasks, imageSize, imageSize, scaledBitmap, segmentColors)
            ...
            return ModelExecutionResult(
                maskImageApplied, scaledBitmap, maskOnly, formatExecutionLog(), itemsFound)
        } catch (e: Exception) {
            val exceptionLog = "something went wrong: ${e.message}"
            Log.d(TAG, exceptionLog)
            val emptyBitmap = ImageUtils.createEmptyBitmap(imageSize, imageSize)
            return ModelExecutionResult(
                emptyBitmap, emptyBitmap, emptyBitmap, exceptionLog, HashMap())
        }
    }
    ...
    private fun getInterpreter(
        context: Context, modelName: String, useGpu: Boolean = false): Interpreter {
    ...
    /**
     * source: https://github.com/tensorflow/tensorflow/blob/master/tensorflow/lite/
     * java/demo/app/src/main/java/com/example/android/tflitecamerademo/ImageClassifier.java
     */
    @Throws(IOException::class)
